...
class InstagramAppUI:
    ...
    def render_about_app_section(self):
        ...
        st.markdown("""
        **Isntsta**는 회원가입이나 결제 없이도 인스타그램에서 **나를 팔로우하지 않는 사람(언팔로워)**을 가장 안전하고 빠르게 찾을 수 있는 웹사이트입니다. 
        """)
        ...
        #### 3. 언팔로워 중에서 대형 크리에이터 등을 걸러낼 수 있는 기능은 없나요?\n
            데이터 분석, 크롤링, API 등의 방법을 고안해봤으나 이들 모두 불가능하거나\n
            규정 위반이기 때문에 해당 기능은 기술적으로 만들기 어렵습니다.\n
